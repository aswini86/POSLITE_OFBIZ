...
public class InvoiceServices {
    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            System.out.println("billToCustomerPartyId----------------------"+orh.getBillToParty().getString("partyId"));
            String billToCustomerPartyId = orh.getBillToParty().getString("partyId");
            //String billToCustomerPartyId = "10010";
            //String billFromVendorPartyId = orh.getBillFromParty().getString("partyId");
            String billFromVendorPartyId = "_NA_";
...
//                }
	...
	public static Map<String, Object> findBillStatus(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        String invoiceId = (String) context.get("invoiceId");
        ...
        if (UtilValidate.isNotEmpty(invoiceId)) {
        	invoiceId = invoiceId.toUpperCase().trim();
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("invoiceId"), EntityOperator.LIKE, EntityFunction.UPPER("%" + invoiceId + "%")));
        }
        
        if (UtilValidate.isNotEmpty(partyId)) {
        	partyId = partyId.toUpperCase().trim();
        ...
        if (UtilValidate.isNotEmpty(partyIdFrom)) {
        	...
        	partyIdFrom = partyIdFrom.toUpperCase().trim();
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyIdFrom"), EntityOperator.EQUALS, partyIdFrom));
       
        }
        
        if (UtilValidate.isNotEmpty(statusId)) {
            
        	statusId = statusId.toUpperCase().trim();
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("statusId"), EntityOperator.EQUALS, statusId));
       
        }
       
       
        mainCond = EntityCondition.makeCondition(andExprs, EntityOperator.AND);
        List<GenericValue> billStatus = null;
        try {
        	billStatus =  EntityQuery.use(delegator).from(entityName).where(mainCond).orderBy("invoiceId", "partyId").queryList();
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
        }
