...
public class TaxAuthorityServices {
    ...
    public static Map<String, Object> rateProductTaxCalc(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String taxCategory = (String) context.get("taxCategory");
        Locale locale = (Locale) context.get("locale");
        ...
        Set<GenericValue> taxAuthoritySet = new HashSet<GenericValue>();
        //Modified by Aswini for calculating IGST and GST
        if (UtilValidate.isNotEmpty(taxCategory) && taxCategory.equals("IGST")) {
        	try {
                getIGSTTaxAuthorities(delegator, shippingAddress, taxAuthoritySet);
            } catch (GenericEntityException e) {
                Debug.logError(e, "Data error getting tax settings: " + e.toString(), module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource, "AccountingTaxSettingError", UtilMisc.toMap("errorString", e.toString()), locale));
            }
        	...
        	try {
        		getGSTTaxAuthorities(delegator, shippingAddress, taxAuthoritySet);
            } catch (GenericEntityException e) {
                Debug.logError(e, "Data error getting tax settings: " + e.toString(), module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource, "AccountingTaxSettingError", UtilMisc.toMap("errorString", e.toString()), locale));
            }
        ...
        if (orderPromotionsAmount != null && orderPromotionsAmount.compareTo(BigDecimal.ZERO) != 0) {
        	//Commented by Aswini as not to calculate Promotion amount on Tax level
    ...
    private static void getGSTTaxAuthorities(Delegator delegator, GenericValue shippingAddress, Set<GenericValue> taxAuthoritySet) throws GenericEntityException {
        Map<String, String> geoIdByTypeMap = new HashMap<String, String>();
        if (shippingAddress != null) {
            if (UtilValidate.isNotEmpty(shippingAddress.getString("stateProvinceGeoId"))) {
                geoIdByTypeMap.put("STATE", shippingAddress.getString("stateProvinceGeoId"));
    ...
    private static void getIGSTTaxAuthorities(Delegator delegator, GenericValue shippingAddress, Set<GenericValue> taxAuthoritySet) throws GenericEntityException {
        Map<String, String> geoIdByTypeMap = new HashMap<String, String>();
        if (shippingAddress != null) {
            if (UtilValidate.isNotEmpty(shippingAddress.getString("countryGeoId"))) {
                geoIdByTypeMap.put("COUNTRY", shippingAddress.getString("countryGeoId"));
        ...
        try {
            ...
            for (GenericValue taxAuthorityRateProduct : lookupList) {
                ...
                if (product != null && taxAuthPartyId != null && taxAuthGeoId != null) {
            		...
            		Debug.logInfo("productId ------########----: " + product.get("productId"),module);
                ...
                if (productPrice != null && "Y".equals(productPrice.getString("taxInPrice")) && itemQuantity != BigDecimal.ZERO) {
                	//Calculate Tax at product level.
                	//(productPrice*(taxpercentage))
                	taxAdjValue.set("orderAdjustmentTypeId", "SALES_TAX");
                	
                	BigDecimal prdTaxAmount = itemPrice.multiply(productPrice.getBigDecimal("taxPercentage").divide(PERCENT_SCALE, 4, BigDecimal.ROUND_HALF_UP)).multiply(itemQuantity);
                    ...
                    taxAdjValue.set("amount", prdTaxAmount);
                } else {
                ...
                taxAdjValue.set("description", taxAuthorityRateProduct.getString("taxAuthorityRateTypeId"));//Added to get tax tye
