...
public class ShoppingCartItem implements java.io.Serializable {
   ...
   public List<GenericValue> getFeaturesForSupplier(LocalDispatcher dispatcher,String partyId) {
    ...
    public BigDecimal getPosLitePrice(String receiptId,String productId) {
        BigDecimal curDisplayPrice;
        String productPrice = "";
        if(productId != null) {
        	try {
				productPrice = EntityQuery.use(delegator).from("PosCartItem").where("receiptId",receiptId,"productId", productId).queryList().
						get(0).getString("productPrice");
			} catch (GenericEntityException e) {
				e.printStackTrace();
			}
        }
        if(productPrice != null) {
        	curDisplayPrice = new BigDecimal(productPrice);
        	return curDisplayPrice;
        }
        if (this.displayPrice == null) {
            curDisplayPrice = this.getBasePrice();
    ...
    public BigDecimal getPosItemSubTotal(String receiptId,BigDecimal quantity) {
        return getPosLitePrice(receiptId,productId).multiply(quantity).multiply(getRentalAdjustment()).add(getOtherAdjustments());
    }
    ...
    public BigDecimal getPosItemSubTotal() {
        return this.getPosItemSubTotal(this.getReceiptId() ,this.getQuantity());
    }
    
    public BigDecimal getListItemSubTotal() {
        return this.getListPrice().multiply(this.getQuantity()).multiply(this.getRentalAdjustment()).add(this.getOtherAdjustments());
    }
    ...
    public BigDecimal getDisplayPosItemSubTotal(String receiptId,String productId) {
        return this.getPosLitePrice(receiptId,productId).multiply(this.getQuantity()).multiply(this.getRentalAdjustment()).add(this.getOtherAdjustments());
    }
