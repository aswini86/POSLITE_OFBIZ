...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
   ...
   public Timestamp getDefaultShipAfterDate() {
    ...
    public BigDecimal getPaymentTotal() {
        ...
        //Modified by Aswini to round up payment total
    ...
    public int selectedPayments() {
    	System.out.println("paymentInfo----"+paymentInfo.size());
    ...
    */
    ...
    public BigDecimal getTotalSalesTax() {
        ...
        for (int i = 0; i < shipInfo.size(); i++) {
            CartShipInfo csi = this.getShipInfo(i);
            System.out.println("csi tax"+csi.getTotalTax(this));
    ...
    public BigDecimal getPosSubTotal(String receiptId) {
        BigDecimal itemsTotal = BigDecimal.ZERO;
        String productId = "";
        for (ShoppingCartItem cartItem : this.cartLines) {
        	productId = cartItem.getProductId();
        	cartItem.setReceiptId(receiptId);
            itemsTotal = itemsTotal.add(cartItem.getPosItemSubTotal());
        }
        return itemsTotal;
    }
    ...
    /*public BigDecimal getGrandTotal() {
    	...
    	//Modified by Aswini to add round up
    	BigDecimal grandTotal = BigDecimal.ZERO;
    	grandTotal = this.getSubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    ...
    public BigDecimal getMRPSubTotal() {
        BigDecimal itemsTotal = BigDecimal.ZERO;
        for (ShoppingCartItem cartItem : this.cartLines) {
            itemsTotal = itemsTotal.add(cartItem.getListItemSubTotal());
        }
        return itemsTotal;
    }
    ...
    public BigDecimal getDisplayPosSubTotal() {
        BigDecimal itemsTotal = BigDecimal.ZERO;
        String productId = "", receiptId = "";
        for (ShoppingCartItem cartItem : this.cartLines) {
        	productId = cartItem.getProductId();
        	receiptId = cartItem.getReceiptId();
            itemsTotal = itemsTotal.add(cartItem.getDisplayPosItemSubTotal(receiptId,productId));
        }
        return itemsTotal;
    }
    ...
    /** Returns the total from the cart, including tax/shipping. */
    public BigDecimal getMRPGrandTotal() {
    	//Modified By Aswini
    	BigDecimal grandTotal = BigDecimal.ZERO;
    	grandTotal = this.getMRPSubTotal().add(this.getTotalShipping()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = grandTotal.setScale(0, BigDecimal.ROUND_HALF_UP);
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
        return grandTotal;
    }
    public BigDecimal getOrgGrandTotal() {
    	//Modified By Aswini
    	BigDecimal grandTotal = BigDecimal.ZERO;
    	grandTotal = this.getDisplaySubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = grandTotal.setScale(0, BigDecimal.ROUND_HALF_UP);
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
        return grandTotal;
    }
    public BigDecimal getDisplayGrandTotal() {
    	...
    	BigDecimal grandTotal = BigDecimal.ZERO;
    	//grandTotal = this.getDisplaySubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = this.getDisplayPosSubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = grandTotal.setScale(0, BigDecimal.ROUND_HALF_UP);
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
        return grandTotal;
    }
    
    public BigDecimal getSellingPriceGrandTotal() {
    	//Modified By Aswini
    	BigDecimal grandTotal = BigDecimal.ZERO;
    	//grandTotal = this.getDisplaySubTotal().add(this.getTotalShipping()).add(this.getTotalSalesTax()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = this.getDisplayPosSubTotal().add(this.getTotalShipping()).add(this.getOrderOtherAdjustmentTotal()).add(this.getOrderGlobalAdjustments());
    	grandTotal = grandTotal.setScale(0, BigDecimal.ROUND_HALF_UP);
    	grandTotal = grandTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
        return grandTotal;
    }
    ...
    //remove duplicate Product Promos
    
    public void getUniqueProductPromos() {
    	...
    	for (ProductPromoUseInfo productPromoUseInfo : this.productPromoUseInfoList) {
    		...
    		//check Product Promo totalCount
    		int count = getProductPromoUseCount(productPromoId);
    	}
    }
    ...
    public void addProductPromoUse(String productPromoId, String productPromoCodeId, BigDecimal totalDiscountAmount, BigDecimal quantityLeftInActions, Map<ShoppingCartItem,BigDecimal> usageInfoMap) {
        ...
        //removeProductPromoUse(productPromoId);//Added By Aswini to remove duplicate calculation of Promos
    ...
    public static class CartShipInfo implements Serializable {
        ...
        public static class CartShipItemInfo implements Serializable {
            ...
            public Map<String, Object> getItemTaxMap(ShoppingCart cart) {
            	HashMap<String, Object> result = new HashMap<String, Object>();
                BigDecimal itemTax = ZERO;

                for (int i = 0; i < itemTaxAdj.size(); i++) {
                    GenericValue v = itemTaxAdj.get(i);
                    itemTax = OrderReadHelper.calcItemAdjustment(v, quantity, item.getBasePrice());
                    itemTax = itemTax.setScale(taxCalcScale, taxRounding);
                    result.put(v.getString("description"), itemTax);
                }

                return result;
            }
            
            public Map<String, Object> getPosItemTaxMap(ShoppingCart cart,String receiptId, String productId) {
            	HashMap<String, Object> result = new HashMap<String, Object>();
                BigDecimal itemTax = ZERO;
                ...
                for (int i = 0; i < itemTaxAdj.size(); i++) {
                    GenericValue v = itemTaxAdj.get(i);
                    //itemTax = OrderReadHelper.calcItemAdjustment(v, quantity, item.getBasePrice());
                    itemTax = OrderReadHelper.calcItemAdjustment(v, quantity, item.getPosLitePrice(receiptId, productId));
                    itemTax = itemTax.setScale(taxCalcScale, taxRounding);
                    result.put(v.getString("description"), itemTax);
                }
