...
public class ShoppingCartEvents {
    ...
    public static String addToCart(HttpServletRequest request, HttpServletResponse response) {
        ...
        String orderId = (String) paramMap.get("posOrderId");
        ...
        request.setAttribute("controlDirective", controlDirective);
    ...
    public static String ShowPosCart(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ...
        if (UtilValidate.isNotEmpty(webPosSession)) {
        	...
        	request.setAttribute("subTotal", shoppingCart.getPosSubTotal(receiptId));
        	...
        	request.setAttribute("displayMRPGrandTotal", shoppingCart.getMRPGrandTotal());
        	request.setAttribute("displayOrgGrandTotal", shoppingCart.getOrgGrandTotal());
        	request.setAttribute("displayGrandTotal", shoppingCart.getDisplayGrandTotal());
        	request.setAttribute("displaySPGrandTotal", shoppingCart.getSellingPriceGrandTotal());
        	request.setAttribute("totalPromotions", shoppingCart.getOrderOtherAdjustmentTotal());
        	
        	request.setAttribute("totalDue", webPosSession.getCurrentTransaction().getPosTotalDue());
        	request.setAttribute("cartLineItem", shoppingCart.size());
        	request.setAttribute("cartSize", shoppingCart.size());
        	
        	System.out.println("totalQuantity---"+shoppingCart.getTotalQuantity());
        	System.out.println("shop size---"+shoppingCart.size());
        	//System.out.println("totalSalesTax&&&&&&---"+shoppingCart.getTotalSalesTax());
        	for (ShoppingCartItem cartLine : shoppingCart) {
        		Map<String, Object> itemTax = new HashMap<String, Object>();
        		Map<String, Object> posItemTax = new HashMap<String, Object>();
        		Map<String, Object> cartLineItem = new HashMap<String, Object>();
        		cartLine.setReceiptId(receiptId);
        		productEan = EntityQuery.use(delegator).from("GoodIdentification").
            			where(UtilMisc.toMap("productId",cartLine.getProductId(),"goodIdentificationTypeId","EAN")).queryFirst();
        		...
        		cartLineItem.put("productOriginalPrice", cartLine.getDisplayPrice());
        		cartLineItem.put("mrp", cartLine.getListPrice());
        		cartLineItem.put("productPrice", cartLine.getPosLitePrice(receiptId,cartLine.getProductId()));
        		cartLineItem.put("productTotalAmt", cartLine.getDisplayPosItemSubTotal(receiptId,cartLine.getProductId()));
        		cartLineItem.put("productAdjustments", cartLine.getAdjustments());
        		if(UtilValidate.isNotEmpty(productEan))
        			cartLineItem.put("barcode", productEan.getString("idValue"));
        		else
        			cartLineItem.put("barcode", "");
        		//Added code to display line item wise tax
        		List<CartShipInfo> shipGroups = shoppingCart.getShipGroups();
        		System.out.println("shipGroups.size()--&&&&&&&&&&&&&&&&&---"+shipGroups.size());
        		if(UtilValidate.isEmpty(cartLine.getAdjustments())) {
        			for (int i = 0; i < shipGroups.size(); i++) {
                        CartShipInfo csi = shoppingCart.getShipInfo(i);
                        CartShipItemInfo cshi = csi.getShipItemInfo(cartLine);
                        itemTax = cshi.getItemTaxMap(shoppingCart);
                    }
        		}
        		if(UtilValidate.isEmpty(cartLine.getAdjustments())) {
        			for (int i = 0; i < shipGroups.size(); i++) {
                        CartShipInfo csi = shoppingCart.getShipInfo(i);
                        CartShipItemInfo cshi = csi.getShipItemInfo(cartLine);
                        posItemTax = cshi.getPosItemTaxMap(shoppingCart, receiptId, cartLine.getProductId());
                    }
        		}
        		System.out.println("itemTax--&&&&&&&&&&&&&&&&&---"+itemTax);
        		System.out.println("posItemTax--&&&&&&&&&&&&&&&&&---"+posItemTax);
        		cartLineItem.put("itemTax", itemTax);
        		cartLineItem.put("posItemTax", posItemTax);
