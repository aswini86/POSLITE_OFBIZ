...
public final class ProductPromoWorker {
    ...
    public static void doPromotions(ShoppingCart cart, List<GenericValue> productPromoList, LocalDispatcher dispatcher) {
        ...
        try {
            ...
            List<String> productPromos = new ArrayList<String>();
            while (promoUses.hasNext()) {
                ProductPromoUseInfo promoUse = promoUses.next();
                //check productPromos contains ProductpromoId
                if(UtilValidate.isEmpty(productPromos)) {
                	sortedPromoUses.add(promoUse);
                	productPromos.add(promoUse.getProductPromoId());
                }else if(UtilValidate.isNotEmpty(productPromos) && !(productPromos.contains(promoUse.getProductPromoId()))) {
                	sortedPromoUses.add(promoUse);
                	productPromos.add(promoUse.getProductPromoId());
                }
            }
    ...
    private static boolean runProductPromoRules(ShoppingCart cart, Long useLimit, boolean requireCode, String productPromoCodeId, Long codeUseLimit, long maxUseLimit,
        ...
        String productPromoId = productPromo.getString("productPromoId");
        System.out.println("useLimit-----------"+useLimit);
        System.out.println("requireCode-----------"+requireCode);
        System.out.println("codeUseLimit-----------"+codeUseLimit);
        System.out.println("cart promo use-----------"+cart.getProductPromoCodeUse(productPromoCodeId));
        System.out.println("cart promoId-----------"+cart.getProductPromoUseCount(productPromoId));
