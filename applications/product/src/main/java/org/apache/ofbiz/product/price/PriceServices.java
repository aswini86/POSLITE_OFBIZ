...
public class PriceServices {
    ...
    public static Map<String, Object> updateProductPriceRuleTrx(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        ...
        String productPriceRuleTxId = (String) context.get("productPriceRuleTxId");
        String ruleName = (String) context.get("ruleName");
        String description = (String) context.get("description");
        Timestamp fromDate = (Timestamp) context.get("fromDate");
        String status = (String) context.get("status");
        String createdBy = (String) context.get("createdBy");
        String productIdArray = (String) context.get("finalPrdIdArray");
        String productPriceActionTypeId = (String) context.get("productPriceActionTypeId");
        ...
        String prdPriceRuleId = "", productPriceRuleId = "";
        String [] productIds = null;
        ArrayList<String> productPriceRuleTxIds = new ArrayList<String>();
        int i=0;
        try {
        	if(UtilValidate.isNotEmpty(productIdArray)) {
        		productIds = productIdArray.split(",");
        	}
        	if(UtilValidate.isNotEmpty(productPriceRuleTxId)) {
            	//Update ProductPriceRule
            	List<GenericValue> prdPriceRuleList = EntityQuery.use(delegator).from("ProductPriceRuleTrx").
            			where("productPriceRuleTxId",productPriceRuleTxId).queryList();
            	if(UtilValidate.isNotEmpty(prdPriceRuleList)) {
            		for (GenericValue prdPriceRuleEach : prdPriceRuleList) {
            			prdPriceRuleId = prdPriceRuleEach.getString("productPriceRuleId");
            			ruleName = "STS"+productIds[i].trim();
            			Map<String, Object> updatePrdPriceRule = dispatcher.runSync("updateProductPriceTrxRule", 
            					UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxId, "productPriceRuleId",prdPriceRuleId,
            							"ruleName",ruleName,"description",description,"fromDate",fromDate,"userLogin", userLogin));
            			
            			productPriceRuleTxIds.add(prdPriceRuleId);
            			if (!ServiceUtil.isError(updatePrdPriceRule)) {
            				
            			}
            			i++;
            		}
            		
            		//Code to update price rule cond.
        			Map<String, Object> updatePrdCondRule = dispatcher.runSync("updateProductPriceCondTrx", 
        					UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxIds,"prdPriceRuleId",prdPriceRuleId,
        							"inputParamEnumId",inputParamEnumId,"operatorEnumId",operatorEnumId,"condValue",condValue,
        							"finalPrdIdArray",productIdArray,"userLogin", userLogin));
        			//Code to update price Action rule.
        			Map<String, Object> updatePrdActionRule = dispatcher.runSync("updateProductPriceActionTrx", 
        					UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxIds,"productPriceActionTypeId",productPriceActionTypeId,
        							"amount",amount,"userLogin", userLogin));
            	}
            }
        	result.put("productPriceRuleId",productPriceRuleTxId);
    ...
    public static Map<String, Object> updateProductPriceActionTrx(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String partyId = userLogin.getString("partyId");
        ...
        ArrayList productPriceRuleTxId = (ArrayList) context.get("productPriceRuleTxId");
        String productPriceActionTypeId = (String) context.get("productPriceActionTypeId");
        Double amount = (Double) context.get("amount");
        String finalRatePerArray = (String) context.get("finalRatePerArray");
        String prdPriceRuleId = "";
        String productPriceActionSeqId = "";
        String [] ratePerVal = null;
        int i=0;
        try {
        	
        	if(UtilValidate.isNotEmpty(productPriceRuleTxId)) {
        		if(UtilValidate.isNotEmpty(finalRatePerArray)) {
        			ratePerVal = finalRatePerArray.split(",");
            	}
            	//Code to update price rule cond.
            	List<GenericValue> prdPriceCondList = EntityQuery.use(delegator).from("ProductPriceTrxAction").
            			where(EntityCondition.makeCondition("productPriceRuleTxId",EntityOperator.IN,productPriceRuleTxId)).queryList();
            	if(UtilValidate.isNotEmpty(prdPriceCondList)) {
            		for (GenericValue prdPriceRuleEach : prdPriceCondList) {
            			prdPriceRuleId = prdPriceRuleEach.getString("productPriceRuleTxId");
            			productPriceActionSeqId = prdPriceRuleEach.getString("productPriceActionSeqId");
            			
            			GenericValue updatePrdActionRule = EntityQuery.use(delegator).from("ProductPriceTrxAction").
            					where("productPriceRuleTxId", prdPriceRuleId, "productPriceActionSeqId",productPriceActionSeqId).queryOne();
            			
            			
            			if(UtilValidate.isNotEmpty(updatePrdActionRule)) {
            				
	            			
            				updatePrdActionRule.set("productPriceActionTypeId", productPriceActionTypeId);
            				if(UtilValidate.isNotEmpty(ratePerVal[i].trim())) {
            					updatePrdActionRule.set("amount", new BigDecimal(ratePerVal[i].trim()));
            				} else {
            					updatePrdActionRule.set("amount", BigDecimal.valueOf(amount));
            				}
            				updatePrdActionRule.store();
            			}
            			i++;
            		}
            	}
    ...
    public static Map<String, Object> updateProductPriceCondTrx(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String partyId = userLogin.getString("partyId");
        ...
        try {
        	...
        	if(UtilValidate.isNotEmpty(productPriceRuleTxId)) {
            	//Code to update price rule action.
            	List<GenericValue> prdPriceCondList = EntityQuery.use(delegator).from("ProductPriceTrxCond").
            			where(EntityCondition.makeCondition("productPriceRuleTxId",EntityOperator.IN,productPriceRuleTxId)).queryList();
            	if(UtilValidate.isNotEmpty(prdPriceCondList)) {
            		for (GenericValue prdPriceRuleEach : prdPriceCondList) {
            			productPriceRuleTxnId = prdPriceRuleEach.getString("productPriceRuleTxId");
            			condValue = productIds[i].trim();
            			productPriceCondSeqId = prdPriceRuleEach.getString("productPriceCondSeqId");
            			
						
						  GenericValue updatePrdCondRule = EntityQuery.use(delegator).from("ProductPriceTrxCond").
						  where("productPriceRuleTxId", productPriceRuleTxnId,
						  "productPriceCondSeqId",productPriceCondSeqId).queryOne();
						 
            			
                        
            			if(UtilValidate.isNotEmpty(updatePrdCondRule)) {
            				
            				updatePrdCondRule.set("inputParamEnumId", inputParamEnumId);
            				updatePrdCondRule.set("operatorEnumId", operatorEnumId);
            				updatePrdCondRule.set("condValue", condValue);
            				updatePrdCondRule.store();
            			}
            			i++;
            		}
            	}
    ...
    public static Map<String, Object> approveProductPriceTxn(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Map<String, Object> result = new HashMap<String, Object>();
        
        String productPriceTxnId = (String) context.get("productPriceTxnId");
        GenericValue productPriceRule = new GenericValue();
        GenericValue productPriceAction = new GenericValue();
        GenericValue productPriceCond = new GenericValue();
        ArrayList<String> productPriceRuleTxnList = new ArrayList<String>();
        try {
        	if (UtilValidate.isNotEmpty(productPriceTxnId)) {
        		//Update ProductPriceTrx
        		GenericValue productPriceTrxList = EntityQuery.use(delegator).from("ProductPriceTrx").
        				where("productPriceRuleTxId",productPriceTxnId).queryOne();
        		if (UtilValidate.isNotEmpty(productPriceTrxList)) {
        			productPriceTrxList.set("status", "APPROVED");
        			productPriceTrxList.store();
        		}
        		//Get ProductPriceRuleTrx entity data
        		List<GenericValue> productPriceRuleTrxList = EntityQuery.use(delegator).from("ProductPriceRuleTrx").
        				where("productPriceRuleTxId",productPriceTxnId).queryList();
        		
        		if(UtilValidate.isNotEmpty(productPriceRuleTrxList)) {
        			//Iterate productPriceRuleTrxList
        			for (GenericValue productPriceRuleTrxListEach: productPriceRuleTrxList) {
        				//Insert records in ProductPriceRule entity
        				productPriceRule = delegator.makeValue("ProductPriceRule", 
        						UtilMisc.toMap("productPriceRuleId",productPriceRuleTrxListEach.getString("productPriceRuleId"),
        								"ruleName",productPriceRuleTrxListEach.getString("ruleName"),
        								"description",productPriceRuleTrxListEach.getString("description"),
        								"fromDate",productPriceRuleTrxListEach.getTimestamp("fromDate")));
        				productPriceRule.create();
        				productPriceRuleTxnList.add(productPriceRuleTrxListEach.getString("productPriceRuleId"));
        			}
        		}
        		//Save ProductPriceAction entity
        		List<GenericValue> productPriceActionTrxList = EntityQuery.use(delegator).from("ProductPriceTrxAction").
        				where(EntityCondition.makeCondition("productPriceRuleTxId",EntityOperator.IN,productPriceRuleTxnList)).queryList();
        		if(UtilValidate.isNotEmpty(productPriceActionTrxList)) {
        			for (GenericValue prdPriceActionTrxEach : productPriceActionTrxList) {
        				
        				productPriceAction = delegator.makeValue("ProductPriceAction", 
        						UtilMisc.toMap("productPriceRuleId",prdPriceActionTrxEach.getString("productPriceRuleTxId"),
        								"productPriceActionSeqId",prdPriceActionTrxEach.getString("productPriceActionSeqId"),
        								"productPriceActionTypeId",prdPriceActionTrxEach.getString("productPriceActionTypeId"),
        								"amount",prdPriceActionTrxEach.getBigDecimal("amount")));
        				productPriceAction.create();
        			}
        		}
        		//save ProductPriceCond
        		List<GenericValue> productPriceCondTrxList = EntityQuery.use(delegator).from("ProductPriceTrxCond").
        				where(EntityCondition.makeCondition("productPriceRuleTxId",EntityOperator.IN,productPriceRuleTxnList)).queryList();
        		if(UtilValidate.isNotEmpty(productPriceCondTrxList)) {
        			for (GenericValue prdPriceCondTrxEach : productPriceCondTrxList) {
        				
        				productPriceCond = delegator.makeValue("ProductPriceCond", 
        						UtilMisc.toMap("productPriceRuleId", prdPriceCondTrxEach.getString("productPriceRuleTxId"),
        								"productPriceCondSeqId", prdPriceCondTrxEach.getString("productPriceCondSeqId"),
        								"inputParamEnumId", prdPriceCondTrxEach.getString("inputParamEnumId"),
        								"operatorEnumId", prdPriceCondTrxEach.getString("operatorEnumId"),
        								"condValue", prdPriceCondTrxEach.getString("condValue")));
        				productPriceCond.create();
        			}
        		}
        ...
        result.put("productPriceTxnId", productPriceTxnId);
        return result;
    }
    
    public static Map<String, Object> saveUpdateProductPriceTxn(DispatchContext dctx, Map<String, ? extends Object> context) {
            Delegator delegator = dctx.getDelegator();
            LocalDispatcher dispatcher = dctx.getDispatcher();
            GenericValue userLogin = (GenericValue) context.get("userLogin");
            String partyId = userLogin.getString("partyId");
            ...
            String prdPriceRuleId = "", productPriceRuleId = "";
            String [] productIds = null;
            String [] ratePercentageVal = null;
            String productPriceRuleTxnId = "";
            ArrayList<String> productPriceRuleTxIds = new ArrayList<String>();
            List<GenericValue> productPriceTrxList = new ArrayList<GenericValue>();
            List<String> prdPriceRuleIds = new ArrayList<String>();
            try {
            	Debug.logInfo("productIdArray--&&&&&&&&&&&&&&&&&&&&&&&&&&&&&--"+productIdArray, module);
            	Debug.logInfo("finalRatePerArray--&&&&&&&&&&&&&&&&&&&&&&&&&&&&&--"+finalRatePerArray, module);
            	if(UtilValidate.isNotEmpty(productIdArray)) {
            		productIds = productIdArray.split(",");
            		ratePercentageVal = finalRatePerArray.split(",");
            		productPriceTrxList = EntityQuery.use(delegator).from("ProductPriceRuleTrx").
                			where("productPriceRuleTxId",productPriceRuleTxId).queryList();
            		if (UtilValidate.isNotEmpty(productPriceTrxList)) {
            			prdPriceRuleIds = EntityUtil.getFieldListFromEntityList(productPriceTrxList, "productPriceRuleId", true);
            		}
            		int j=0;
            		for (String productId : productIds) {
            			amount = Double.valueOf(ratePercentageVal[j]);
            			//check product exists or not for that rule.
            			EntityCondition prdPriceTxnCondition = EntityCondition.makeCondition(EntityOperator.AND,
            					EntityCondition.makeCondition("productPriceRuleTxId",EntityOperator.IN,prdPriceRuleIds),
            					EntityCondition.makeCondition("condValue",EntityOperator.EQUALS,productId.trim()));
            			
            			Debug.logInfo("prdPriceTxnCondition--&&&&&&&&&&&&&&&&&&&&&&&&&&&&&--"+prdPriceTxnCondition, module);
                    	List<GenericValue> productPriceTrxCondList = EntityQuery.use(delegator).from("ProductPriceTrxCond").
                    			where(prdPriceTxnCondition).queryList();
                    	if(UtilValidate.isEmpty(productPriceTrxCondList)) {
                    		Debug.logInfo("is Empty--&&&&&&&&&&&&&&&&&&&&&&&&&&&&&--"+productPriceRuleTxId, module);
                    		//create ProductPriceRuleTrx
                    		Map<String, Object> createProductPriceRuleTrxMap = dispatcher.runSync("createProductPriceTrxRule", 
                    				UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxId, "ruleName","STS"+productId.trim(),
                    						"description",description,"fromDate",fromDate,"status",status,"createdBy",createdBy,
                    						"userLogin", userLogin));
                    		productPriceRuleTxnId = (String) createProductPriceRuleTrxMap.get("productPriceRuleId");
                    		//create productPriceCond
                    		Map<String, Object> createProductPriceCondTrxMap = dispatcher.runSync("createProductPriceTrxCond", 
                    				UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxnId, "inputParamEnumId",inputParamEnumId,
                    						"operatorEnumId",operatorEnumId,"condValue",productId.trim(),"userLogin", userLogin));
                    		//create productPriceAction
                    		Map<String, Object> createProductPriceActionTrxMap = dispatcher.runSync("createProductPriceTrxAction", 
                    				UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxnId,
                    						"productPriceActionTypeId",productPriceActionTypeId,
                    						"amount",BigDecimal.valueOf(amount),"userLogin", userLogin));
                    	} else {
                    		//Code to update
                    		if(UtilValidate.isNotEmpty(productPriceRuleTxId)) {
                            	//Update ProductPriceRule
                            	...
                            	if(UtilValidate.isNotEmpty(prdPriceRuleList)) {
                        			...
                        			Map<String, Object> updatePrdCondRule = dispatcher.runSync("updateProductPriceCondTrx", 
                        					UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxIds,"prdPriceRuleId",prdPriceRuleId,
                        							"inputParamEnumId",inputParamEnumId,"operatorEnumId",operatorEnumId,"condValue",productId,
                        							"finalPrdIdArray",productIdArray,"userLogin", userLogin));
                        			//Code to update price Action rule.
                        			Map<String, Object> updatePrdActionRule = dispatcher.runSync("updateProductPriceActionTrx", 
                        					UtilMisc.toMap("productPriceRuleTxId", productPriceRuleTxIds,"productPriceActionTypeId",productPriceActionTypeId,
                        							"finalRatePerArray",finalRatePerArray,"userLogin", userLogin));
                            	}
                            }
    ...
    public static Map<String, Object> createInventoryVarianceTxn(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String partyId = userLogin.getString("partyId");
        ...
        String varReasonIdArray = (String) context.get("varReasonIdArray");
        String inventoryItemArray = (String) context.get("inventoryItemArray");
        String quantityArray = (String) context.get("quantityArray");
        
        String [] inventoryItemIds = null;
        String [] varReasonIds = null;
        String [] quantityVal = null;
        
        String invItemTrxId = "";
        try {
        	//create InventoryItemTrx
        	invItemTrxId = delegator.getNextSeqId("InventoryItemTrx");
        	GenericValue createInventoryItemTrx = delegator.makeValue("InventoryItemTrx", 
        			UtilMisc.toMap("inventoryItemTxId",invItemTrxId, "description", comments, "status", status,
        					"createdBy", createdBy, "createdDate", UtilDateTime.nowTimestamp()));
        	createInventoryItemTrx.create();
        	
        	if(UtilValidate.isNotEmpty(inventoryItemArray)) {
        		inventoryItemIds = inventoryItemArray.split(",");
        		varReasonIds = varReasonIdArray.split(",");
        		quantityVal = quantityArray.split(",");
        		
        		int i = 0;
        		for (String inventoryItemId : inventoryItemIds) {
        			//create InventoryItemVarianceTrx.
        			
        			GenericValue createInventoryItemVarTrx = delegator.makeValue("InventoryItemVarianceTrx", 
                			UtilMisc.toMap("inventoryItemTxId",invItemTrxId, "inventoryItemId", inventoryItemId,
                					"varianceReasonId", varReasonIds[i], "availableToPromiseVar", quantityVal[i],
                					"quantityOnHandVar", quantityVal[i], "comments", comments, "status", status, 
                					"createdBy", createdBy, "createdDate", UtilDateTime.nowTimestamp()));
        			createInventoryItemVarTrx.create();
        			i++;
        		}
        	}
        	result.put("inventoryItemTxId",invItemTrxId);
        } catch(GenericEntityException  e) {
    ...
    public static Map<String, Object> createUpdateInventoryVarianceTxn(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String partyId = userLogin.getString("partyId");
        ...
        String invItemTrxId = (String) context.get("invItemTrxId");
        String comments = (String) context.get("comments");
        String status = (String) context.get("status");
        String createdBy = (String) context.get("createdBy");
        ...
        String [] varReasonIds = null;
        String [] quantityVal = null;
        GenericValue InventoryItemVarianceTrxVal = new GenericValue();
        try {
        	...
        	if(UtilValidate.isNotEmpty(inventoryItemArray)) {
        		...
        		int i = 0;
        		for (String inventoryItemId : inventoryItemIds) {
        			//check inventoryItemId exists in InventoryItemVarianceTrx
        			InventoryItemVarianceTrxVal = EntityQuery.use(delegator).from("InventoryItemVarianceTrx").
        					where("inventoryItemTxId",invItemTrxId,"inventoryItemId",inventoryItemId).queryOne();
        			
        			if (UtilValidate.isNotEmpty(InventoryItemVarianceTrxVal)) {
        				//if exists update
        				InventoryItemVarianceTrxVal.set("comments", comments);
        				InventoryItemVarianceTrxVal.set("status", status);
        				InventoryItemVarianceTrxVal.set("updatedBy", createdBy);
        				InventoryItemVarianceTrxVal.set("availableToPromiseVar", quantityVal[i]);
        				InventoryItemVarianceTrxVal.set("quantityOnHandVar", quantityVal[i]);
        				InventoryItemVarianceTrxVal.store();
        			} else {
        				//create InventoryItemVarianceTrx.
            			GenericValue createInventoryItemVarTrx = delegator.makeValue("InventoryItemVarianceTrx", 
                    			UtilMisc.toMap("inventoryItemTxId",invItemTrxId, "inventoryItemId", inventoryItemId,
    ...
    public static Map<String, Object> approveInventoryVarianceTxn(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        String partyId = userLogin.getString("partyId");
        ...
        String invItemTrxId = (String) context.get("invItemTrxId");
        String inventoryItemId = "",comments = "", availableToPromiseVar = "";
        String quantityOnHandVar = "", varianceReasonId = "", createdBy = "";
        GenericValue InventoryItemTrxVal = new GenericValue();
        List<GenericValue> InventoryItemVarianceTrxList = new ArrayList<GenericValue>();
        try {
        	
        	if(UtilValidate.isNotEmpty(invItemTrxId)) {
    			//check inventoryItemId exists in InventoryItemVarianceTrx
        		InventoryItemTrxVal = EntityQuery.use(delegator).from("InventoryItemTrx").where("inventoryItemTxId",invItemTrxId).queryOne();
    			
    			if (UtilValidate.isNotEmpty(InventoryItemTrxVal)) {
    				//Approve Inventory Item variance
    				InventoryItemTrxVal.set("updatedBy", partyId);
    				InventoryItemTrxVal.set("status", "APPROVED");
    				InventoryItemTrxVal.store();
    				//get InventoryItemVarianceTrx list
    				InventoryItemVarianceTrxList = EntityQuery.use(delegator).from("InventoryItemVarianceTrx").
    						where("inventoryItemTxId",invItemTrxId).queryList();
    				
    				for (GenericValue invItemVarRow : InventoryItemVarianceTrxList) {
    					
    					inventoryItemId = invItemVarRow.getString("inventoryItemId");
    					comments = invItemVarRow.getString("comments");
    					availableToPromiseVar = invItemVarRow.getString("availableToPromiseVar");
    					quantityOnHandVar = invItemVarRow.getString("quantityOnHandVar");
    					varianceReasonId = invItemVarRow.getString("varianceReasonId");
    					createdBy = invItemVarRow.getString("createdBy");
    					...
    					Map<String, Object> map = dispatcher.runSync("createInventoryItemVariance", 
    							UtilMisc.toMap("inventoryItemId",inventoryItemId, "comments", comments, "status","APPROVED",
    					...
    					invItemVarRow.set("status", "APPROVED");
    					invItemVarRow.set("updatedBy", partyId);
    					invItemVarRow.store();
    				}
    			}
        	}
