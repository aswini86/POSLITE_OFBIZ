...
public final class ProductStoreWorker {
    ...
    public static String getProductStoreId(ServletRequest request) {
        ...
        //Modified code as per POS
        GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");
        if (userLogin == null) {
            userLogin = (GenericValue) session.getAttribute("autoUserLogin");
        } 
        if (userLogin == null) {
        	String username = (String) request.getAttribute("login.username");
            if (username == null) username = (String) request.getAttribute("username");
            if (username == null) username = (String) request.getParameter("USERNAME");
            try {
        ...
        GenericValue productStore = null;
        try {
            productStore = EntityQuery.use(delegator).from("ProductStoreRole").where("partyId", partyId).orderBy("createdStamp").queryFirst();
        ...
        //End of product store code.
        if (UtilValidate.isNotEmpty(productStoreId)) {
            return productStoreId;
        }else if (session != null && session.getAttribute("productStoreId") != null) {
