...
public class PartyServices {
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        // get the role types
        Map<String, Object> partyAttrmap = new HashMap<String, Object>();
        try {
        ...
        if ("Y".equals(lookupFlag)) {
            ...
            if (!"Y".equals(showAll)) {
              ...
              //Code to get Party Attribute List
				/*
				 * dynamicView.addMemberEntity("PA", "PartyAttribute");
				 * dynamicView.addAlias("PA", "attrName"); dynamicView.addAlias("PA",
				 * "attrValue"); dynamicView.addViewLink("PT", "PA", Boolean.FALSE,
				 * ModelKeyMap.makeKeyMapList("partyId"));
				 * 
				 * fieldsToSelect.add("attrName"); fieldsToSelect.add("attrValue");
				 */                // ----
                // Person Fields
                // ----
            ...
            if (mainCond != null || "Y".equals(showAll)) {
                try {
                  ...
                  //Added code to get partyAttributes
                    try {
                    	if(UtilValidate.isNotEmpty(partyList)) {
                    		for (GenericValue partyListEach:partyList) {
                    			String partyIdVal = partyListEach.getString("partyId");
                            	
                            	List<GenericValue> partyAttrList = EntityQuery.use(delegator).from("PartyAttribute")
                                        .where("partyId", partyIdVal).cache().queryList();
                            	partyAttrmap.put(partyIdVal,partyAttrList);
                            	System.out.println("partyAttrmap-----"+partyAttrmap);
                    		}
                    	}
                    } catch (GenericEntityException e) {
                    	
                    }
                    //End of code tot get partyAttributes
        ...
        result.put("partyAttrmap", partyAttrmap);
    ...
    public static Map<String, Object> importParty(DispatchContext dctx, Map<String, Object> context) {
		...
		String mobile = null;
        String emailAddressAtr = null;
        String pan = null;
		String gstinType = null;
		String gstin = null;
		String fssaiNumber = null;
		String fssaiExpiryDate = null;
		String orderRequired = null;
        ...
        try {
            for (final CSVRecord rec : fmt.parse(csvReader)) {
              ...
              //PartyAttr data insert
                if(UtilValidate.isNotEmpty(rec.get("GSTNO"))) {
            		gstNo = rec.get("GSTNO");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","GSTNO","attrValue",gstNo,
					  "userLogin", userLogin));
            	}
                
                if(UtilValidate.isNotEmpty(rec.get("VENDOR_NAME"))) {
                	vendorName = rec.get("VENDOR_NAME");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Vendor_Name","attrValue",vendorName,
					  "userLogin", userLogin));
            	}
                if(UtilValidate.isNotEmpty(rec.get("ADDRESS_1"))) {
                	address1 = rec.get("ADDRESS_1");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Address_1","attrValue",address1,
					  "userLogin", userLogin));
            	}
                if(UtilValidate.isNotEmpty(rec.get("ADDRESS_2"))) {
                	address2 = rec.get("ADDRESS_2");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Address_2","attrValue",address2,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("ADDRESS_3"))) {
                	address3 = rec.get("ADDRESS_3");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Address_3","attrValue",address3,
					  "userLogin", userLogin));
            	}
					if(UtilValidate.isNotEmpty(rec.get("COUNTRY"))) {
                	country = rec.get("COUNTRY");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","COUNTRY","attrValue",country,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("STATES"))) {
                	states = rec.get("STATES");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","STATES","attrValue",states,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("CITY"))) {
                	city = rec.get("CITY");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","CITY","attrValue",city,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("PinCode"))) {
                	pinCode = rec.get("PinCode");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","PinCode","attrValue",pinCode,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("ContactPerson"))) {
                	contactPerson = rec.get("ContactPerson");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","ContactPerson","attrValue",contactPerson,
					  "userLogin", userLogin));
            	}
                if(UtilValidate.isNotEmpty(rec.get("Landline"))) {
                	landLine = rec.get("Landline");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Landline","attrValue",landLine,
					  "userLogin", userLogin));
            	}
				 if(UtilValidate.isNotEmpty(rec.get("Mobile"))) {
                	mobile = rec.get("Mobile");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","Mobile","attrValue",mobile,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("EmailAddress"))) {
					emailAddressAtr = rec.get("EmailAddress");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","EmailAddress","attrValue",emailAddressAtr,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("PAN"))) {
                	pan = rec.get("PAN");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","PAN","attrValue",pan,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("GSTINType"))) {
                	gstinType = rec.get("GSTINType");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","GSTINType","attrValue",gstinType,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("GSTIN"))) {
                	gstin = rec.get("GSTIN");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","GSTIN","attrValue",gstin,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("FSSAINumber"))) {
                	fssaiNumber = rec.get("FSSAINumber");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","FSSAINumber","attrValue",fssaiNumber,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("FSSAIExpiryDate"))) {
                	fssaiExpiryDate = rec.get("FSSAIExpiryDate");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","FSSAIExpiryDate","attrValue",fssaiExpiryDate,
					  "userLogin", userLogin));
            	}
				if(UtilValidate.isNotEmpty(rec.get("OrderRequired"))) {
                	orderRequired = rec.get("OrderRequired");
            		//call the partyAttr service
					  Map<String,Object> partyAttrMap = dispatcher.runSync("createPartyAttribute",
					  UtilMisc.toMap("partyId",newPartyId,"attrName","OrderRequired","attrValue",orderRequired,
					  "userLogin", userLogin));
            	}
    ...
    public Map<String, Object> findFacilityInvItem(DispatchContext dispatchContext, Map<String, ? extends Object> context) {
        ...
        try {
        	...
        	if (UtilValidate.isNotEmpty(facilityId)) {
        		...
        		productList = EntityQuery.use(delegator).from("Product").where(
        				EntityCondition.makeCondition("productId",EntityOperator.IN,productIds)).orderBy("productId").distinct(true).queryList();
        	}else {
        		productList = EntityQuery.use(delegator).from("Product").orderBy("productId").distinct(true).queryList();
        	}
			/*
			 * if (UtilValidate.isNotEmpty(productId)) { productList =
			 * EntityQuery.use(delegator).from("InventoryItem").where("productId",productId)
			 * .orderBy("productId").distinct(true).queryList(); }
			 */
            
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
        }
        result.put("listIt", productList);
        return result;
    }
    
    public Map<String, Object> exportProductEANCategory(DispatchContext dispatchContext, Map<String, ? extends Object> context) {
        Delegator delegator = dispatchContext.getDelegator();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Map<String, Object> result = ServiceUtil.returnSuccess();
        
        List<GenericValue> productList = new ArrayList<GenericValue>();
        List<Map<String, Object>> productFinalList = new ArrayList<Map<String,Object>>();
        String productId = "", productName = "";
        String primaryParentCategoryId = "";
        GenericValue prdSubCategoryVal = new GenericValue();
        GenericValue prdCategoryVal = new GenericValue();
        try {
    		//Get Product list
    		productList = EntityQuery.use(delegator).from("Product").orderBy("productId").distinct(true).queryList();
    		if(UtilValidate.isNotEmpty(productList)) {
    			for (GenericValue prdRow : productList) {
    				Map<String, Object> productMap = new HashMap<String, Object>();
    				productId = prdRow.getString("productId");
    				productName = prdRow.getString("productName");
    				
    				productMap.put("productId", productId);
    				productMap.put("productName", productName);
    				//Get Product EAN
    				GenericValue goodIdentificationVal = EntityQuery.use(delegator).from("GoodIdentification").
    						where("goodIdentificationTypeId","EAN", "productId", productId).queryOne();
    				if(UtilValidate.isNotEmpty(goodIdentificationVal)) {
    					productMap.put("EAN", goodIdentificationVal.getString("idValue"));
    				} else {
    					productMap.put("EAN", "");
    				}
    				GenericValue productCategory = EntityQuery.use(delegator).from("ProductCategoryMember").
    						where("productId",productId).orderBy("-createdStamp").queryFirst();
    				
    				if (UtilValidate.isNotEmpty(productCategory)) {
    					
    					List<GenericValue> prdCatList = EntityQuery.use(delegator).from("ProductCategory").
    							where("productCategoryId",productCategory.getString("productCategoryId")).queryList();
    					if(UtilValidate.isNotEmpty(prdCatList)) {//get Product Main Category
    						
    						if(UtilValidate.isNotEmpty(prdCatList.get(0).getString("primaryParentCategoryId"))) {
    							primaryParentCategoryId = prdCatList.get(0).getString("primaryParentCategoryId");
    							
    							prdCategoryVal = EntityQuery.use(delegator).from("ProductCategory").
            							where("productCategoryId",primaryParentCategoryId).queryOne();
    							
    							productMap.put("CategoryName", prdCategoryVal.getString("categoryName"));
    						} else {
    							productMap.put("CategoryName", "");
    						}
    						if(UtilValidate.isNotEmpty(prdCatList.get(0).getString("productCategoryId"))) {
    							prdSubCategoryVal = EntityQuery.use(delegator).from("ProductCategory").
            							where("productCategoryId",prdCatList.get(0).getString("productCategoryId")).queryOne();
    							
    							productMap.put("SubCategoryName", prdSubCategoryVal.getString("categoryName"));
    						} else {
    							productMap.put("SubCategoryName", "");
    						}
    					}
    				} else {
    					productMap.put("CategoryName", "");
    				...
    				productFinalList.add(productMap);
    			}
    		}
        ...
        result.put("listIt", productFinalList);
        return result;
    }
}