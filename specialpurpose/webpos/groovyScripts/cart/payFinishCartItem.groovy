/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
import org.apache.ofbiz.base.util.UtilValidate
import org.apache.ofbiz.entity.util.EntityQuery
import org.apache.ofbiz.webpos.WebPosEvents
import org.apache.ofbiz.webpos.session.WebPosSession
import org.apache.ofbiz.order.shoppingcart.ShoppingCartItem
import org.apache.ofbiz.service.GenericServiceException;
import java.util.List
import org.apache.ofbiz.base.util.UtilValidate
import org.apache.ofbiz.base.util.UtilMisc
import org.apache.ofbiz.order.shoppingcart.ShoppingCart
import org.apache.ofbiz.order.shoppingcart.ShoppingCartEvents
import org.apache.ofbiz.entity.GenericEntityException
import org.apache.ofbiz.entity.GenericValue;
import org.apache.ofbiz.entity.condition.EntityCondition;
import org.apache.ofbiz.entity.util.EntityQuery;
import org.apache.ofbiz.entity.condition.EntityOperator;
import org.apache.ofbiz.base.util.Debug;
import org.apache.ofbiz.base.util.UtilDateTime
import org.apache.ofbiz.entity.Delegator;
import javax.servlet.http.HttpSession
import org.apache.ofbiz.webpos.transaction.WebPosTransaction
import org.apache.ofbiz.order.shoppingcart.CheckOutHelper
import java.sql.Timestamp

HttpSession session = request.getSession(true);
session.removeAttribute("shoppingCart");
session.removeAttribute("webPosSession");

webPosSession = WebPosEvents.getWebPosSession(request, null);
List<GenericValue> posCartItemList = new ArrayList<GenericValue>();
GenericValue posCartItemData = new GenericValue();
List<Map<String, Object>> cartItems = new ArrayList<Map<String, Object>>();
if (webPosSession) {
	println("came into web pos add cart item--------&&&&&&&&&&&&&&&&&&&&_----------------");
	shoppingCart = webPosSession.getCart();
	String receiptId = request.getParameter("receiptId");
	String posTerminalId = request.getParameter("posTerminalId");
	String dayId = request.getParameter("dayId");
	String customerShipState = request.getParameter("customerShipState")
	String username = request.getParameter("USERNAME")
	String amountCash = request.getParameter("amountCash");
	String paymentType = request.getParameter("paymentType");
	String paymentTransaction = request.getParameter("paymentTransaction");
	BigDecimal cashToPay = new BigDecimal(amountCash);
	String refNum = ""
	String authCode = ""
	String totalDue = ""
	String facilityFromState = ""
	BigDecimal amount = BigDecimal.ZERO;
	BigDecimal balanceAmount = BigDecimal.ZERO
	BigDecimal totalBillAmount = BigDecimal.ZERO
	BigDecimal paidAmount = BigDecimal.ZERO
	Timestamp now = UtilDateTime.nowTimestamp()
	BigDecimal totalSpDue = BigDecimal.ZERO
	BigDecimal cartItemQty = BigDecimal.ZERO;
	BigDecimal cartItemPrdPrice = BigDecimal.ZERO;
	String facilityId = "", productId = ""
	String partyId = ""
	println("amountCash-------${amountCash}");
	if (UtilValidate.isNotEmpty(receiptId) && UtilValidate.isNotEmpty(amountCash)) {
		ShoppingCart shCart = new ShoppingCart(shoppingCart);
		//get userLogin
		userLogin = EntityQuery.use(delegator).from("UserLogin").where("userLoginId", username).cache(false).queryOne();
		shCart.setUserLogin(userLogin)
		webPosSession.setUserLogin(userLogin)
		try {
			//Check the value exists or not
			posCartItemList = EntityQuery.use(delegator).from("PosCartItem")
					.where("receiptId", receiptId, "dayId", dayId).queryList();
			
			if (UtilValidate.isNotEmpty(posCartItemList)) {
				for (GenericValue posCartItemEach : posCartItemList) {
					
					productId = posCartItemEach.getString("productId");
					quantity = posCartItemEach.getString("quantity");
					terminalId = posCartItemEach.getString("terminalId");
					println("add_product_id---each&&&&&&&&&7---"+productId);
					
					request.setAttribute("posTerminalId", terminalId)
					request.setAttribute("quantity", quantity)
					request.setAttribute("add_product_id", productId)
					request.setAttribute("posOrderId", receiptId)
					
					ShoppingCartEvents.addToCart(request,response)
					
					//calc tax
					//get user partyId
					String contactMechId = ""
					userLogin = EntityQuery.use(delegator).from("UserLogin").where("userLoginId", username).queryOne();
					partyId = userLogin != null ? userLogin.getString("partyId") : null;
					println("partyId-------------${partyId}");
					//get party facility
					partyFacilityList = EntityQuery.use(delegator).from("FacilityParty")
									.where("partyId", partyId).queryList();
					//get facility postal contact mech id
					if(UtilValidate.isNotEmpty(partyFacilityList)) {
						facilityId = partyFacilityList.get(0).getString("facilityId");
						facilityContactList = EntityQuery.use(delegator).from("FacilityAndContactMech")
						.where("facilityId", facilityId).queryList();
						contactMechId = facilityContactList.get(0).getString("contactMechId");
						println("contactMechId-------------${contactMechId}");
					}
					shipAddress = EntityQuery.use(delegator).from("PostalAddress")
									.where(EntityCondition.makeCondition("contactMechId", EntityOperator.EQUALS, contactMechId)).queryOne();
					if(UtilValidate.isNotEmpty(shipAddress)) {
						facilityFromState = shipAddress.getString("stateProvinceGeoId");
					}
					CheckOutHelper coh = new CheckOutHelper(dispatcher, delegator, shoppingCart);
					if (facilityFromState != customerShipState) {
						//Need to add IGST tax logic.
						println("came into calc IGST-----&&&&&&&&&&&&&&&--------------");
						coh.calcAndAddTax(shipAddress,"IGST")
					} else {
						coh.calcAndAddTax(shipAddress)
					}
					
					//ShoppingCartEvents.ShowPosCart(request,response)
				}
			}
		} catch (GenericEntityException e) {
			Debug.logError(e, "Error while updating Pos Cart Item", module);
		}
		
		try {
			//Code for calculating pos cash
			WebPosTransaction wpt = new WebPosTransaction(webPosSession);
			if(paymentType.equals("CASH")) {
				wpt.addPayment(paymentType, cashToPay, refNum, authCode)
			}
			if(paymentType.equals("RAJOR_PAY")) {
				wpt.addPayment(paymentType, cashToPay, refNum, authCode)
			}
			if(paymentType.equals("Mswipe")) {
				wpt.addPayment(paymentType, cashToPay, refNum, authCode)
			}
			
			//paymentType-Rajorpay-cash-transaction
			totalDue = webPosSession.getCurrentTransaction().getTotalDue();
			request.setAttribute("totalDue", totalDue)
			if (shoppingCart) {
				shoppingCartSize = shoppingCart.size();
				println("shoppingcart size------------${shoppingCartSize}")
				payments = shoppingCart.selectedPayments();
				println("payments------------${payments}")
				for (i = 0; i < payments; i++) {
					paymentInfo = shoppingCart.getPaymentInfo(i);
					println("paymentInfo---------${paymentInfo}")
					if (paymentInfo.amount != null) {
						amount = paymentInfo.amount;
						println("amount---------${amount}")
					}
				}
			}
			//End of code for calculating pos cash
			
			WebPosEvents posEvent = new WebPosEvents();
			
			String oId = shCart.getOrderId();
			totalSpDue = shCart.getSellingPriceGrandTotal()
			//String comSale = posEvent.completeSale(request, response);
			//create orderHeader
			GenericValue createOrderHeader = delegator.makeValue("OrderHeader", UtilMisc.toMap("orderId",receiptId, 
					"orderTypeId", "SALES_ORDER","salesChannelEnumId","POS_SALES_CHANNEL",
					"orderDate", now,"entryDate", now,"statusId","ORDER_COMPLETED","createdBy",username,
					"currencyUom","INR","originFacilityId",facilityId,"terminalId",posTerminalId,
					"remainingSubTotal",new BigDecimal(amount),"grandTotal",new BigDecimal(amount)));

			createOrderHeader.create();
			//create orderItem
			posCartItemList = EntityQuery.use(delegator).from("PosCartItem").where("receiptId", receiptId, "dayId", dayId).queryList();
			if (UtilValidate.isNotEmpty(posCartItemList)) {
				int orderItemSeqId = 00001
				for (GenericValue posCartItemEach : posCartItemList) {
					productId = posCartItemEach.getString("productId");
					quantity = posCartItemEach.getString("quantity");
					productPrice = posCartItemEach.getString("productPrice");
					if(UtilValidate.isNotEmpty(quantity)) {
						cartItemQty = new BigDecimal(quantity)
					}
					if(UtilValidate.isNotEmpty(productPrice)) {
						cartItemPrdPrice = new BigDecimal(productPrice)
					}
					GenericValue createOrderItem = delegator.makeValue("OrderItem", UtilMisc.toMap("orderId", receiptId,
						"orderItemSeqId", orderItemSeqId.toString(),"orderItemTypeId", "PRODUCT_ORDER_ITEM","productId",productId,"isPromo","N",
						"quantity", cartItemQty,"unitPrice", cartItemPrdPrice,"unitListPrice", cartItemPrdPrice,"isModifiedPrice","N",
						"itemDescription", productId,"statusId","ITEM_COMPLETED"));
			
					createOrderItem.create();
					orderItemSeqId++
					println("orderItemSeqId---------${orderItemSeqId}")
					println("facilityId---------${facilityId}")
					println("productId---------${productId}")
					//update inventory stock
					GenericValue invItemVal = EntityQuery.use(delegator).from("InventoryItem").where("facilityId", facilityId,
						"productId", productId.trim()).queryFirst();
					println("invItemVal-----7777777777777777----${invItemVal}")
					if(UtilValidate.isNotEmpty(invItemVal)) {
						//Create ItemInvemtory variance
						inventoryItemId = invItemVal.getString("inventoryItemId");
						println("cartItemQty-----7777777777777777----${cartItemQty}")
						try {
							Map<String,Object> physicalInvMap = dispatcher.runSync("createPhysicalInventory",
									UtilMisc.toMap("partyId", partyId, "physicalInventoryDate", UtilDateTime.nowTimestamp(),
											"userLogin", userLogin));
							
							Map<String, Object> map = dispatcher.runSync("createInventoryItemVariance",
									UtilMisc.toMap("inventoryItemId",inventoryItemId, "comments", "", "status","APPROVED",
											"quantityOnHandVar", cartItemQty.negate(),
											"availableToPromiseVar", cartItemQty.negate(),
											"physicalInventoryId", physicalInvMap.get("physicalInventoryId"),
											"userLogin", userLogin));
						} catch (GenericServiceException e) {
							
						}
					}
					//End of update of inventory stock
				}
			}
			
			//Update PosCart status to COMPLETE
			try {
				posCartTxnData = EntityQuery.use(delegator).from("PosCartTransaction").
									where("receiptId", receiptId).queryOne();
									
				if (UtilValidate.isNotEmpty(posCartTxnData)) {
					totalBillAmount = posCartTxnData.getBigDecimal("totalBillAmount")
					paidAmount = posCartTxnData.getBigDecimal("paidAmount")
					if(UtilValidate.isNotEmpty(paidAmount)) {
						balanceAmount = totalBillAmount.subtract(paidAmount)
					} else {
						balanceAmount = totalBillAmount.subtract(cashToPay)
					}
					
					posCartTxnData.set("posStatus", "COMPLETE")
					posCartTxnData.set("paidAmount", cashToPay);
					posCartTxnData.set("balanceAmount", balanceAmount);
					posCartTxnData.set("paymentType", paymentType);
					//posCartTxnData.set("paymentTransaction", paymentTransaction);
					posCartTxnData.store()
				}
									
				posCartItemList = EntityQuery.use(delegator).from("PosCartItem")
						.where("receiptId", receiptId, "dayId", dayId).queryList();
				
				if (UtilValidate.isNotEmpty(posCartItemList)) {
					for (GenericValue posCartItemEach : posCartItemList) {
						posCartItemEach.set("posStatus", "COMPLETE")
						posCartItemEach.store()
					}
				}
				//code to create sync
				Map<String, Object> createSyncProduct = dispatcher.runSync("createCartTransaction", UtilMisc.toMap("receiptId", receiptId,
					"userLogin", userLogin));
				//end of code to create sync
				request.setAttribute("receiptId", receiptId)
				request.setAttribute("successMsg", "Cart Checkout done Successfully")
			} catch (GenericEntityException e) {
				
			}
		} catch (GenericEntityException e) {
			Debug.logError(e, "Error while creating Cart Checkout", module);
		}
	}
	
} else {
	shoppingCart = null;
}

if (shoppingCart) {
	
	return "Success"
} else {
	context.shoppingCartSize = 0;
}



