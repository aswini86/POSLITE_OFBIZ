...
if (webPosSession) {
	...
	if (UtilValidate.isNotEmpty(barcode)) {
		...
		if(UtilValidate.isEmpty(productEanList)) {
			...
			Map<String, Object> createProductMap = dispatcher.runSync("createGlobalProduct", UtilMisc.toMap("barcode", barcode, 
				"productName", productName, "mrp", mrp, "sp", sp, "userLogin", userLogin));
			productId = (String) createProductMap.get("productId");
			println("productId--&&&&&&-------------${productId}")
		} else {
	...
	if (UtilValidate.isNotEmpty(receiptId)) {
		...
		try {
			...
			if (UtilValidate.isNotEmpty(posCartItemData)) {
				...
				posCartItemData.set("productPrice", sp)
				...
				if (UtilValidate.isNotEmpty(receiptId)) {
					posCartItem = delegator.makeValue("PosCartItem", UtilMisc.toMap("receiptId", receiptId,
						...
						"terminalId",posTerminalId, "posStatus", "CREATED", "productPrice", sp));
					posCartItem.create()
				}
			}
