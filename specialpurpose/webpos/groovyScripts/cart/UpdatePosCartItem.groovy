...
result = [:]
println("came into update cart items--&&&&&&-------------")
if (webPosSession) {
	...
	BigDecimal displayOrgGrandTotal = BigDecimal.ZERO
	BigDecimal balanceAmount = BigDecimal.ZERO
	BigDecimal paidAmt = BigDecimal.ZERO
	BigDecimal displayMRPGrandTotal = BigDecimal.ZERO
	float discPer = (float) 0.0
	float discAmount = (float) 0.0
	
	if (UtilValidate.isNotEmpty(barcode)) {
		productEanList = EntityQuery.use(delegator).from("GoodIdentification").where("goodIdentificationTypeId", "EAN",
							"idValue", barcode).queryList();
		
		productId = productEanList.get(0).getString("productId");
	}
	...
	if (UtilValidate.isNotEmpty(receiptId)) {
		...
		try {
			...
			if (UtilValidate.isNotEmpty(posCartItemData)) {
				...
				updatedQty = Integer.parseInt(quantity)
				println("totQty----------${updatedQty}")
				println("price----------${price}")
				posCartItemData.set("quantity", updatedQty)
				posCartItemData.set("productPrice", price)
	...
	if (UtilValidate.isNotEmpty(posCartItemList)) {
		...
		if(UtilValidate.isNotEmpty(posCartTxnData)) {
			displayOrgGrandTotal = shoppingCart.getOrgGrandTotal()
			displayGrandTotal = shoppingCart.getSellingPriceGrandTotal()
			displayMRPGrandTotal = shoppingCart.getMRPGrandTotal()
			...
			discPer = Math.round((displayOrgGrandTotal.floatValue() - displayGrandTotal.floatValue()) / (displayOrgGrandTotal.floatValue()) * ((float) 100))
			discAmount = (displayMRPGrandTotal.floatValue() - displayGrandTotal.floatValue())
			println("discPer-------&&--------------${discPer}")
			...
			request.setAttribute("discPer", discPer)
			if(discAmount >= 0)
				request.setAttribute("discAmount", discAmount)
			else
				request.setAttribute("discAmount", 0.0f)
