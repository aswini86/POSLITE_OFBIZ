...
if (webPosSession) {
	...
	String paidAmount = request.getParameter("posPaidAmount");
	...
	BigDecimal bpaidAmt = BigDecimal.ZERO
	BigDecimal totalBillAmount = BigDecimal.ZERO
	BigDecimal balanceAmount = BigDecimal.ZERO
	if (UtilValidate.isNotEmpty(receiptId)) {
		
		try {
			//Check the PosCart txn exists or not
			if(UtilValidate.isNotEmpty(paidAmount)) {
				bpaidAmt = new BigDecimal(paidAmount)
			}
			...
			if(UtilValidate.isNotEmpty(posCartTxnData)) {
				...
				println("paidAmount-----------------${bpaidAmt}")
				balanceAmount = totalBillAmount.subtract(bpaidAmt)
				println("balanceAmount-----------------${balanceAmount}")
				posCartTxnData.set("posStatus", "HOLD");
				posCartTxnData.set("paidAmount", bpaidAmt);
				posCartTxnData.set("balanceAmount", balanceAmount);
				posCartTxnData.store()
				request.setAttribute("paidAmount", bpaidAmt)
				request.setAttribute("balanceAmount", balanceAmount)
				
				if (bpaidAmt.compareTo(totalBillAmount) == 1) {
					returnAmount = bpaidAmt.subtract(totalBillAmount)
					request.setAttribute("returnAmount", returnAmount)
				} else {
					request.setAttribute("returnAmount", BigDecimal.ZERO)
				}
			}
			//Check the PosCartItem exists or not
			posCartItemList = EntityQuery.use(delegator).from("PosCartItem")
					.where("receiptId", receiptId, "dayId", dayId).queryList();
			//if value exists update the value in pos cart item and update as Hold
			if (UtilValidate.isNotEmpty(posCartItemList)) {
				for (GenericValue posCartItemEach : posCartItemList) {
					println("came into update----------");
					posCartItemEach.set("posStatus", "HOLD")
					println("posCartItemData----------${posCartItemEach}");
			...
			if (UtilValidate.isNotEmpty(paymentType)) {
				...
				println("came into create Payment----------")
				...
				cartPayment = delegator.makeValue("PosCartReceivePayment", UtilMisc.toMap("posCartPaymentId",posCartPaymentId,
					"receiptId",receiptId,"dayId",dayId,"terminalId",posTerminalId,"paymentType",paymentType,"receivedPayment",bpaidAmt))
				cartPayment.create();
			}
